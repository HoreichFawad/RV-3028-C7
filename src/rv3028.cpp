...
bool RV3028::begin(bool set_24Hour, bool disable_TrickleCharge, bool set_LevelSwitchingMode, bool reset_Status)
{
...
uint8_t RV3028::readRegister(uint8_t addr)
{
	...
	// }
    int rc=_i2cPort->read(RV3028_ADDR<<1, &pBuf,1,false);
      if (rc != 0)
    {
...
bool RV3028::writeRegister(uint8_t addr, uint8_t val)
{
	...
	_i2cPort->write(RV3028_ADDR<<1);
	_i2cPort->write(addr);
	bool ackReceived=_i2cPort->write(val);
    _i2cPort->stop();
    _i2cPort->unlock();
    if(ackReceived)
	    return(true);
...
bool RV3028::readMultipleRegisters(uint8_t addr, uint8_t * dest, uint8_t len)
{
	...
	// return(true);
    int rc=_i2cPort->read(RV3028_ADDR<<1, (char*)dest, len,false);
    if(rc!=0)
        return false;
    else
	    return true;
...
bool RV3028::writeMultipleRegisters(uint8_t addr, uint8_t * values, uint8_t len)
{
    ...
    _i2cPort->start();
	_i2cPort->write(RV3028_ADDR<<1);
	bool ackReceived=_i2cPort->write(addr);
	for (uint8_t i = 0; i < len; i++)
	{
...
bool RV3028::waitforEEPROM()
{
    Timer t;
    t.start();
	unsigned long timeout = t.read_ms() + 500;
	while ((readRegister(RV3028_STATUS) & 1 << STATUS_EEBUSY) && t.read_ms() < timeout);
